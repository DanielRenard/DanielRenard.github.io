export const profile = {
  name: "Daniel Renard",
  title:
    "M.A. History | Library Associate | Full-Stack Engineer: React | Node | AWS",
  blurb:
    "I bridge the humanities and technology—bringing experience in broadcasting, research, and software engineering to create accessible, engaging learning and digital environments.",
  location: "Southern California",
};

export const education = [
  {
    school: "California State Polytechnic University-Pomona / History",
    credential: "Master of Arts",
    details: [
      "Research Skills",
      "Broad subject range meant to prepare an educator",
    ],
  },
  {
    school: "University of Louisiana at Lafayette / Education",
    details: ["Lesson Planning", "Pedagogy"],
  },
  {
    school: "University of New Orleans / Film, Theatre, and Communication Arts",
    credential: "Bachelor of Arts",
    details: [
      "Video Production, Documentary filming, Script Writing, Sound Production, Short film Prodution",
    ],
  },
];

export const training = [
  {
    org: "University of Louisiana at Lafayette  / Institute of Data",
    program:
      "Software Engineering / Certificate of Completion & Certificate of Achievement",
    dates: "2023",
    details: [
      "Front End Development: HTML, CSS, Figma, Bootstrap, MUI, Javascript Programing;",
      "Back-End Development: Database design, MongoDB, MySQL, Redis;",
      "Software Development: Docker, CI/CD With GitHub Actions, EC2, BeanStalk, Node.js & React, API Development",
    ],
  },
];

export const projects = [
  {
    name: "Clarisphere: A Coaching Application",
    description:
      "Working with Quantum Sky Inc (a small startup), we have designed an engaging online coaching platform. The backend is currently in development, but you can take a peek at the frontend code now.",
    tech: ["React", "MUI", "Vite", "Node", "AWS"],
    links: [
      {
        label: "GitHub",
        href: "https://github.com/QuantumSkyInc/project-steve-fe",
      },
    ],
  },
  {
    name: "Shot List: News Production",
    description:
      "Capstone Project: The Shot List app was made to be the first place Directors and Producers go to transmit shot composition ideas when working on live studio-based broadcasts. Shot List can also store all previously held compositions for quick access when stacking a show. ",
    tech: ["JavaScript", "CSS", "HTML", "React", "Node", "MUI", "MongoDB"],
    links: [
      {
        label: "GitHub",
        href: "https://github.com/DanielRenard/CapstoneProjectShotList",
      },
    ],
  },
  {
    name: "Dice Roller",
    description: "Simple JavaScript exercise",
    tech: ["JavaScript", "CSS", "HTML"],
    links: [
      { label: "GitHub", href: "https://github.com/DanielRenard/diceRoller" },
    ],
  },
  {
    name: "E-commerce Website",
    description: "Simple Storefront exercise",
    tech: ["JavaScript", "CSS", "HTML", "Bootstrap"],
    links: [
      { label: "GitHub", href: "https://github.com/DanielRenard/Module4Lab10" },
    ],
  },
];

export const experience = [
  {
    role: "Library Associate II",
    company: "Riverside County Library System",
    dates: "2025 – Current",
    bullets: [
      "Research and exhibition of archived items",
      "Public Service & Reference: Provide in-depth assistance to patrons with library materials, research, and technology (computers, scanners, copiers).",
      "Circulation & Records: Manage complex circulation tasks, such as handling overdue fines, managing hold requests, and maintaining patron records.",
      "Collection Maintenance: Oversee shelving, shelf-reading for accuracy, and identifying items needing repair or withdrawal.",
      "Programming & Outreach: Assist with or lead library events, storytimes, and promotional activities like creating displays or flyers.",
      "Supervision & Operations: Supervise library assistants, pages, or volunteers; assist in opening/closing procedures.",
      "Technical Support: Assist with basic cataloging or database maintenance.",
    ],
  },
  {
    role: "Trainer Assistant ",
    company: "Institute of Data",
    dates: "2024 – 2026",
    bullets: [
      "Assist in Curriculum Delivery: Help the lead trainer deliver lessons, workshops, or training sessions, ensuring students understand technical concepts. Break down complex topics into digestible pieces for learners of varying skill levels. ",
      "Provide Technical Support: Assist students with hands-on coding exercises, debugging issues, and understanding best practices. Demonstrate how to use tools, frameworks, or software development methodologies effectively. ",
      "Mentoring and Guidance: Support students in developing problem-solving and analytical skills. Offer feedback on assignments, projects, or code quality to help learners improve.",
      "Content Preparation and Adaptation: Adapt teaching methods based on feedback or the specific needs of learners.",
      "Monitoring Progress: Track student progress and engagement, identifying those who might need additional support. Provide reports or updates to the lead trainer or program management.",
      "Facilitating Communication: Act as a bridge between the lead trainer and students, ensuring open communication and clarity. Answer student questions during or after sessions to ensure learning continuity.",
    ],
  },
  {
    role: "Newscast Director",
    company: "KLFY News 10/Nexstar ",
    dates: "2017 – 2024",
    bullets: [
      "Directed daily live newscasts, coordinating staff and technology under strict deadlines.",
      "Assisted colleagues in operating production systems and solved technical issues in real time.",
      "Maintained accurate programming records and ensured compliance with broadcast standards.",
      "Recognized with multiple LAB Prestige Awards for excellence in broadcast execution and teamwork.",
    ],
  },
  {
    role: "Working Lead/Photographer",
    company: "Disneyland Resort ",
    dates: "2011 – 2015",
    bullets: [
      "Delivered exceptional customer service by creating unique experiences for diverse guests.",
      "Scheduled and supervised photographers, balancing staffing needs with guest demand.",
      "Resolved guest concerns promptly and professionally, ensuring satisfaction.",
      "Maintained equipment, organized schedules, and trained new employees in operational procedures.",
    ],
  },
];

export const contact = {
  email: "danieljuderenard@gmail.com",
  linkedin: "https://www.linkedin.com/in/daniel-renard-420037a9/",
  github: "https://github.com/DanielRenard",
  youtube: "https://www.youtube.com/channel/UC3kWtsJTNsPsCRWaA7PTA_Q",
  youtubePlaylistId: "PLH4WqIE-aKbzX14gwKyAMiB0CI4yUAWwF",
};
